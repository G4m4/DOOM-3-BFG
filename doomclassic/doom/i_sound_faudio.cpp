...
#define FAILED(hr) (((unsigned)(hr)) < 0)
...
int I_StartSound2 ( int id, int player, mobj_t *origin, mobj_t *listener_origin, int pitch, int priority ) {
	...
	if ( origin && origin != listener_origin ) {
		...
		unsigned dwCalculateFlags = F3DAUDIO_CALCULATE_MATRIX;
		F3DAudioCalculate( F3DAudioInstance, &doom_Listener, &sound->m_Emitter, dwCalculateFlags, &sound->m_DSPSettings );
	...
	// Submit packet
	unsigned hr;
	if( FAILED( hr = FAudioSourceVoice_SubmitSourceBuffer( sound->m_pSourceVoice, &Packet, NULL ) ) ) {
		int fail = 1;
	}

	// Play the source voice
	if( FAILED( hr = FAudioSourceVoice_Start( sound->m_pSourceVoice, 0, FAUDIO_COMMIT_NOW ) ) ) {
		int fail = 1;
	}
...
void I_InitSoundChannel( int channel, int numOutputChannels_ ) {
	...
	FAudio_CreateSourceVoice( soundSystemLocal.hardware.GetIXAudio2(), &soundchannel->m_pSourceVoice, (const FAudioWaveFormatEx *)&voiceFormat, 0, 2.0f, NULL, NULL, NULL );
}
...
void I_InitMusic(void)		
{
	if ( !Music_initialized ) {
		...
		FAudio_CreateSourceVoice(soundSystemLocal.hardware.GetIXAudio2(), &pMusicSourceVoice, (const FAudioWaveFormatEx *)&voiceFormat, FAUDIO_VOICE_MUSIC, 2.0f, NULL, NULL, NULL );
...
void I_ShutdownMusic(void)	
{
	...
	if ( Music_initialized ) {
		...
		if ( hMusicThread ) {
			pthread_join(hMusicThread, NULL);
		}
		if ( musicBuffer ) {
			free( musicBuffer );
		}
...
======================
*/
unsigned  I_LoadSong( void* songname ) {
	idStr lumpName = "d_";
	...
	if ( waitingForMusic ) {
		...
		if ( musicReady && pMusicSourceVoice != NULL ) {
			...
			if ( musicBuffer ) {
				...
				// Submit packet
				unsigned hr;
				if( FAILED( hr = FAudioSourceVoice_SubmitSourceBuffer( pMusicSourceVoice, &Packet, NULL ) ) ) {
					int fail = 1;
				}

				// Play the source voice
				if( FAILED( hr = FAudioSourceVoice_Start( pMusicSourceVoice, 0, FAUDIO_COMMIT_NOW ) ) ) {
					int fail = 1;
				}
			}
