...
/*
========================
idSoundHardware_stub::idSoundHardware_stub
========================
*/
idSoundHardware_stub::idSoundHardware_stub() {
	pAudioStub = NULL;
...
void listDevices_f( const idCmdArgs & args ) {
	...
	// if ( stubGetDeviceCount( &deviceCount ) != 0 || deviceCount == 0 ) {
	// 	idLib::Warning( "No audio devices found" );
	// 	return;
	// }

	// for ( unsigned int i = 0; i < deviceCount; i++ ) {
	// 	stubDeviceDetails deviceDetails;
	// 	if ( stubGetDeviceDetails( i, &deviceDetails ) != 0 ) {
	// 		continue;
	// 	}
	// 	idStaticList< const char *, 5 > roles;
	// 	if ( deviceDetails.Role & stubDefaultConsoleDevice ) {
	// 		roles.Append( "Console Device" );
	// 	}
	// 	if ( deviceDetails.Role & stubDefaultMultimediaDevice ) {
	// 		roles.Append( "Multimedia Device" );
	// 	}
	// 	if ( deviceDetails.Role & stubDefaultCommunicationsDevice ) {
	// 		roles.Append( "Communications Device" );
	// 	}
	// 	if ( deviceDetails.Role & stubDefaultGameDevice ) {
...
/*
========================
idSoundHardware_stub::Init
========================
*/
void idSoundHardware_stub::Init() {

	cmdSystem->AddCommand( "listDevices", listDevices_f, 0, "Lists the connected sound devices", NULL );

	uint32_t xAudioCreateFlags = 0;
#ifdef _DEBUG
	xAudioCreateFlags |= STUB_DEBUG_ENGINE;
#endif

// 	stubProcessor xAudioProcessor = STUB_DEFAULT_PROCESSOR;

// 	if ( FAILED( stubCreate( &pAudioStub, xAudioCreateFlags, xAudioProcessor ) ) ) {
// 		if ( xAudioCreateFlags & STUB_DEBUG_ENGINE ) {
// 			// in case the debug engine isn't installed
// 			xAudioCreateFlags &= ~STUB_DEBUG_ENGINE;
// 			if ( FAILED( XAudio2Create( &pAudioStub, xAudioCreateFlags, xAudioProcessor ) ) ) {		
...
// 	}
// #ifdef _DEBUG
// 	STUB_DEBUG_CONFIGURATION debugConfiguration = { 0 };
// 	debugConfiguration.TraceMask = STUB_LOG_WARNINGS;
// 	debugConfiguration.BreakMask = STUB_LOG_ERRORS;
// 	stubSetDebugConfiguration( &debugConfiguration );
// #endif

// 	// Register the sound engine callback
// 	stubRegisterForCallbacks( &soundEngineCallback );
// 	soundEngineCallback.hardware = this;

// 	uint32_t deviceCount = 0;
// 	if ( stubGetDeviceCount( &deviceCount ) != S_OK || deviceCount == 0 ) {
// 		idLib::Warning( "No audio devices found" );
// 		stubRelease();
// 		pAudioStub = NULL;
// 		return;
...
// 		for ( unsigned int i = 0; i < deviceCount; i++ ) {
// 			stubDeviceDetails deviceDetails;
// 			if ( stubGetDeviceDetails( i, &deviceDetails ) != S_OK ) {
// 				continue;
// 			}
...
// 	stubDeviceDetails deviceDetails;
// 	if ( stubGetDeviceDetails( preferredDevice, &deviceDetails ) != S_OK ) {
...
// 	uint32_t outputSampleRate = 44100; // Max( (uint32_t)STUBFX_REVERB_MIN_FRAMERATE, Min( (uint32_t)STUBFX_REVERB_MAX_FRAMERATE, deviceDetails.OutputFormat.Format.nSamplesPerSec ) );

// 	if ( FAILED( stubCreateMasteringVoice( &pMasterVoice, STUB_DEFAULT_CHANNELS, outputSampleRate, 0, preferredDevice, NULL ) ) ) {
...
// 	}
	...
	// // ---------------------
	// FAPO * vuMeter = NULL;
	// stubCreateVolumeMeter( &vuMeter, 0 );
	...
	// // ---------------------
	// if ( FAILED( stubCreateSubmixVoice( &pSubmixVoice, 1, outputSampleRate, 0, 0, NULL, NULL ) ) ) {
...
/*
========================
idSoundHardware_stub::Shutdown
========================
*/
void idSoundHardware_stub::Shutdown() {
	for ( int i = 0; i < voices.Num(); i++ ) {
	...
	// stubUnregisterForCallbacks( &soundEngineCallback );
...
/*
========================
idSoundHardware_stub::AllocateVoice
========================
*/
idSoundVoice * idSoundHardware_stub::AllocateVoice( const idSoundSample * leadinSample, const idSoundSample * loopingSample ) {
	...
	for ( int i = 0; i < freeVoices.Num(); i++ ) {
		...
		if ( voice->CompatibleFormat( (idSoundSample_stub*)leadinSample ) ) {
			break;
		}
	}
	if ( voice != NULL ) {
...
/*
========================
idSoundHardware_stub::FreeVoice
========================
*/
void idSoundHardware_stub::FreeVoice( idSoundVoice * voice ) {
	voice->Stop();
...
/*
========================
idSoundHardware_stub::Update
========================
*/
void idSoundHardware_stub::Update() {
	// if ( pAudioStub == NULL ) {
	...
	// stubCommitChanges( XAUDIO2_COMMIT_ALL );
