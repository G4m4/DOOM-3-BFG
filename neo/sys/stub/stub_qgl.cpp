...
void QGL_Shutdown( void )
{
	...
	// if ( stubInstance.hinstOpenGL )
	// {
	// 	common->Printf( "...unloading OpenGL DLL\n" );
	// 	FreeLibrary( stubInstance.hinstOpenGL );
	// }

	// stubInstance.hinstOpenGL = NULL;
...
#define GPA( a ) GetProcAddress( stubInstance.hinstOpenGL, a )
...
{
	// assert( stubInstance.hinstOpenGL == 0 );

	// common->Printf( "...initializing QGL\n" );

	// common->Printf( "...calling LoadLibrary( '%s' ): ", dllname );

	// if ( ( stubInstance.hinstOpenGL = LoadLibrary( dllname ) ) == 0 )
	// {
