...
void Sys_ShutdownInput() {
	// IN_DeactivateMouse();
	// IN_DeactivateKeyboard();
	// if ( stubInstance.g_pKeyboard ) {
	// 	stubInstance.g_pKeyboard->Release();
	// 	stubInstance.g_pKeyboard = NULL;
	// }

    // if ( stubInstance.g_pMouse ) {
	// 	stubInstance.g_pMouse->Release();
	// 	stubInstance.g_pMouse = NULL;
	// }

    // if ( stubInstance.g_pdi ) {
	// 	stubInstance.g_pdi->Release();
	// 	stubInstance.g_pdi = NULL;
	// }
}
...
void Sys_InitInput() {
	...
	IN_InitDirectInput();
	if ( stubInstance.in_mouse.GetBool() ) {
		IN_InitDIMouse();
	...
	common->Printf ("------------------------------------\n");
	stubInstance.in_mouse.ClearModified();
}
...
void IN_Frame() {
	...
	if ( !stubInstance.cdsFullscreen ) {
		if ( stubInstance.mouseReleased ) {
			shouldGrab = false;
		}
		if ( stubInstance.movingWindow ) {
			shouldGrab = false;
		}
		if ( !stubInstance.activeApp ) {
			shouldGrab = false;
		}
	}

	if ( shouldGrab != stubInstance.mouseGrabbed ) {
		if ( usercmdGen != NULL ) {
			usercmdGen->Clear();
		}

		if ( stubInstance.mouseGrabbed ) {
			IN_DeactivateMouse();
		} else {
			IN_ActivateMouse();
			...
			if ( !IN_InitDIMouse() ) {
				stubInstance.in_mouse.SetBool( false );
				return;
			}
#endif
		}
...
void	Sys_GrabMouseCursor( bool grabIt ) {
	stubInstance.mouseReleased = !grabIt;
	if ( !grabIt ) {
...
int Sys_PollKeyboardInputEvents() {
    ...
    // if( stubInstance.g_pKeyboard == NULL ) {
    //     return 0;
	// }
    
    // dwElements = DINPUT_BUFFERSIZE;
    // hr = stubInstance.g_pKeyboard->GetDeviceData( sizeof(DIDEVICEOBJECTDATA),
    ...
    //     hr = stubInstance.g_pKeyboard->Acquire();
	...
	// 		//some problems.
	// 		stubInstance.g_pKeyboard->GetDeviceData( sizeof(DIDEVICEOBJECTDATA), NULL, &dwElements, 0 );
	// 		dwElements = 0;
	// 	}
...
int Sys_PollKeyboardInputEvents() {
	...
	hr = stubInstance.g_pKeyboard->GetDeviceState( sizeof( toggleFetch[ diFetch ] ), toggleFetch[ diFetch ] );
    if( hr != DI_OK ) 
    {
        // We got an error or we got DI_BUFFEROVERFLOW.
        //
...
int Sys_PollMouseInputEvents( int mouseEvents[MAX_MOUSE_EVENTS][2] ) {
	...
	// if ( !stubInstance.g_pMouse || !stubInstance.mouseGrabbed ) {
	// 	return 0;
	// }

    // dwElements = DINPUT_BUFFERSIZE;
    // hr = stubInstance.g_pMouse->GetDeviceData( sizeof(DIDEVICEOBJECTDATA), polled_didod, &dwElements, 0 );

    // if( hr != DI_OK ) {
    //     hr = stubInstance.g_pMouse->Acquire();
	// 	// clear the garbage
	// 	if (!FAILED(hr)) {
	// 		stubInstance.g_pMouse->GetDeviceData( sizeof(DIDEVICEOBJECTDATA), polled_didod, &dwElements, 0 );
	// 	}
    // }
...
// 			idScopedCriticalSection cs( stubInstance.g_Joystick.mutexXis );

// 			for ( int i = 0 ; i < MAX_JOYSTICKS ; i++ ) {
// 				controllerState_t * cs = &stubInstance.g_Joystick.controllers[i];
...
// 		WaitForSingleObject( stubInstance.g_Joystick.timer, INFINITE );
	}
}
...
/*
========================
idJoystickStub::SetRumble
========================
*/
void idJoystickStub::SetRumble( int inputDeviceNum, int rumbleLow, int rumbleHigh ) {
...
/*
========================
idJoystickStub::PostInputEvent
========================
*/
void idJoystickStub::PostInputEvent( int inputDeviceNum, int event, int value, int range ) {
...
/*
========================
idJoystickStub::PollInputEvents
========================
*/
int idJoystickStub::PollInputEvents( int inputDeviceNum ) {
	numEvents = 0;
...
/*
========================
idJoystickStub::ReturnInputEvent
========================
*/
int idJoystickStub::ReturnInputEvent( const int n, int & action, int &value ) {
...
/*
========================
idJoystickStub::PushButton
========================
*/
void idJoystickStub::PushButton( int inputDeviceNum, int key, bool value ) {
