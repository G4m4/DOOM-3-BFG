...
idCVar SDLVars_t::sdl_allowAltTab( "win_allowAltTab", "0", CVAR_SYSTEM | CVAR_BOOL, "allow Alt-Tab when fullscreen" );
idCVar SDLVars_t::sdl_notaskkeys( "win_notaskkeys", "0", CVAR_SYSTEM | CVAR_INTEGER, "disable windows task keys" );
idCVar SDLVars_t::sdl_username( "win_username", "", CVAR_SYSTEM | CVAR_INIT, "windows user name" );
idCVar SDLVars_t::sdl_outputEditString( "win_outputEditString", "1", CVAR_SYSTEM | CVAR_BOOL, "" );
idCVar SDLVars_t::sdl_viewlog( "win_viewlog", "0", CVAR_SYSTEM | CVAR_INTEGER, "" );
idCVar SDLVars_t::sdl_timerUpdate( "win_timerUpdate", "0", CVAR_SYSTEM | CVAR_BOOL, "allows the game to be updated while dragging the window" );
idCVar SDLVars_t::sdl_allowMultipleInstances( "win_allowMultipleInstances", "0", CVAR_SYSTEM | CVAR_BOOL, "allow multiple instances running concurrently" );

SDLVars_t sdlInstance;
...
void Sys_Printf(const char* fmt, ...) {
	...
	if (sdlInstance.sdl_outputEditString.GetBool() && idLib::IsMainThread()) {
...
const char* Sys_Cwd() {
	...
	memcpy(cwd, SDL_GetCurrentDirectory(), strlen(SDL_GetCurrentDirectory()) - 1);
	cwd[MAX_OSPATH - 1] = 0;
...
const char* Sys_GetProcessorString() {
	return sdlInstance.sys_cpustring.GetString();
}
...
void Win_Frame() {
	...
	if (sdlInstance.sdl_viewlog.IsModified()) {
		sdlInstance.sdl_viewlog.ClearModified();
	}
}
...
int main(int argc, char** argv) {
	...
	if (sdlInstance.sdl_notaskkeys.GetInteger()) {
		// 	DisableTaskKeys( TRUE, FALSE, /*( win32.win_notaskkeys.GetInteger() == 2 )*/ FALSE );
	}

	// hide or show the early console as necessary
	if (sdlInstance.sdl_viewlog.GetInteger()) {
		Sys_ShowConsole(1, true);
	}
	else {
